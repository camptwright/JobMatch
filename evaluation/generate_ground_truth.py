...
def score_with_openai(resume_text: str, job_title: str, job_description: str, model: str = "gpt-4o-mini") -> int:
    import openai
...
def score_with_anthropic(resume_text: str, job_title: str, job_description: str, model: str = "claude-sonnet-4-6") -> int:
    import anthropic
...
def generate_ground_truth(
    ...
    print(f"Selected {len(sampled_resumes)} query resumes across {len(categories)} categories")
    ...
    for qi, resume in enumerate(sampled_resumes):
        ...
        print(f"\n[{qi+1}/{len(sampled_resumes)}] Resume {resume_id} ({resume_cat}) -> {len(candidates)} candidates")
        ...
        for ji, (_, job) in enumerate(candidates.iterrows()):
            ...
            if (ji + 1) % 5 == 0:
                print(f"    Scored {ji+1}/{len(candidates)} pairs")
    ...
    for grade in range(4):
        ...
        print(f"    Grade {grade}: {count} ({count/total_pairs*100:.1f}%)")
    print(f"{'='*50}")
...
def generate_category_ground_truth(
    ...
    total = len(judgments)
    print(f"Category-based ground truth saved: {output_path}")
    print(f"  Total pairs: {total}")
    for grade in [0, 1, 3]:
        count = sum(1 for j in judgments if j['relevance'] == grade)
        print(f"  Grade {grade}: {count} ({count/total*100:.1f}%)")


def generate_pooled_ground_truth(
    resumes_csv: str,
    jobs_csv: str,
    output_path: str,
    bm25f_index_path: str,
    num_queries: int = 50,
    top_k: int = 20,
    ...
    print(f"Loading BM25F index from {bm25f_index_path}...")
    idx = BM25FIndex.load(bm25f_index_path)
    print(f"Loaded {idx.N:,} docs")
    ...
    print(f"Selected {len(sampled)} query resumes")
    ...
    for qi, resume in enumerate(sampled):
        ...
        if (qi + 1) % 10 == 0 or qi == 0:
            print(f"  [{qi+1}/{len(sampled)}] Resume {resume_id} ({resume_cat}): {len(candidate_ids)} candidates")
    ...
    print(f"\nPooled ground truth saved: {output_path}")
    print(f"  Total pairs: {total}, Relevant: {relevant} ({relevant/total*100:.1f}%)")

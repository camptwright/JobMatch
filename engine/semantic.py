...
class SemanticIndex:
    ...
    def save(self, directory: str):
        ...
        with open(os.path.join(directory, 'meta.json'), 'w') as f:
            json.dump(meta, f)
        print(f"Semantic index saved to {directory}/")

    @classmethod
    def load(cls, directory: str) -> 'SemanticIndex':
...
def build_job_semantic_index(
    ...
    if sample_size and sample_size < len(df):
        ...
        print(f"Subsampled to {sample_size} jobs for semantic index")
...
def build_resume_semantic_index(
    resumes_csv: str,
    model_name: str = 'all-MiniLM-L6-v2',
    device: str = None,
) -> SemanticIndex:
    ...
    try:
        ...
        for q in queries:
            ...
            for rank, (doc_id, score) in enumerate(results, 1):
                ...
                print(f"  {rank}. [{score:.3f}] {m.get('title', doc_id)}")

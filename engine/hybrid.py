...
class HybridRetriever:
    ...
    def search(
        ...
        if mode == 'bm25f':
            ...
            if not return_metadata:
                return results
            return [
                (doc_id, score, self.bm25f.get_doc(doc_id))
        ...
        if mode == 'semantic':
            ...
            if not return_metadata:
                return results
            return [
                (doc_id, score, self.semantic.get_doc(doc_id))
                for doc_id, score in results
            ]
    ...
    def compare_modes(self, query: str, top_k: int = 5) -> Dict[str, List]:
        return {
...
if __name__ == '__main__':
    ...
    try:
        ...
        for rank, (doc_id, score) in enumerate(results, 1):
            print(f"  {rank}. [{score:.3f}] {bm25.get_doc(doc_id)['title']}")

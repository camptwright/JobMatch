...
def load_arshkon():
	print('Loading arshkon postings from {}...'.format(ARSHKON_POSTINGS))
	df = pd.read_csv(ARSHKON_POSTINGS)
	print('  Loaded {:,} rows, columns: {}...'.format(len(df), list(df.columns[:8])))
...
def load_asaniczka():
	print('Loading asaniczka postings from {}...'.format(ASANICZKA_POSTINGS))
	...
	print('  Using columns: {}'.format(available))
	...
	if os.path.exists(ASANICZKA_SKILLS):
		print('Loading asaniczka skills from {}...'.format(ASANICZKA_SKILLS))
		skills_df = pd.read_csv(ASANICZKA_SKILLS)
		print('  Loaded {:,} skill entries'.format(len(skills_df)))

	return df, skills_df


def load_snehaanbhawal():
	print('Loading snehaanbhawal resumes from {}...'.format(SNEHAANBHAWAL_RESUMES))
	df = pd.read_csv(SNEHAANBHAWAL_RESUMES)
	print('  Loaded {:,} rows, columns: {}'.format(len(df), list(df.columns)))
...
def load_florex():
	print('Loading florex resumes from {}...'.format(FLOREX_RESUMES))
	records = []
	...
	print('  Loaded {:,} resumes'.format(len(df)))
	return df
...
def fig2_job_description_lengths(arshkon_df):
	...
	ax.axvline(median_val, color=C_RED, linestyle='--', linewidth=1.2,
		label='Median: {} words'.format(median_val))
	ax.axvline(mean_val, color=C_ORANGE, linestyle=':', linewidth=1.2,
		label='Mean: {} words'.format(mean_val))
	ax.set_xlabel('Word Count')
	ax.set_ylabel('Frequency')
	ax.set_title('Job Description Length Distribution (arshkon, n={:,})'.format(len(wc)))
...
def fig3_resume_lengths(sneha_df, florex_df):
	...
	ax1.axvline(int(wc1.median()), color=C_RED, linestyle='--', linewidth=1,
		label='Median: {}'.format(int(wc1.median())))
	ax1.set_xlabel('Word Count')
	ax1.set_ylabel('Frequency')
	ax1.set_title('snehaanbhawal (n={:,})'.format(len(wc1)))
	...
	ax2.axvline(int(wc2.median()), color=C_BLUE, linestyle='--', linewidth=1,
		label='Median: {}'.format(int(wc2.median())))
	ax2.set_xlabel('Word Count')
	ax2.set_title('florex (n={:,})'.format(len(wc2)))
...
def fig4_resume_categories(sneha_df):
	...
	ax.barh(range(len(cats)), cats.values, color=C_ORANGE, alpha=0.85, height=0.7)
	ax.set_yticks(range(len(cats)))
	ax.set_yticklabels(cats.index, fontsize=8)
	ax.invert_yaxis()
	ax.set_xlabel('Count')
	ax.set_title('Resume Categories -- snehaanbhawal (n={:,})'.format(len(sneha_df)))
	ax.spines['top'].set_visible(False)
	ax.spines['right'].set_visible(False)
	plt.tight_layout()
...
def fig5_top_skills(skills_df):
	...
	if skill_col is None:
		print('  WARNING: Could not find skill column. Columns are: {}'.format(list(skills_df.columns)))
	...
	fig, ax = plt.subplots(figsize=(7, 4.5))
	ax.barh(range(len(top)), top.values, color=C_GREEN, alpha=0.85, height=0.7)
	ax.set_yticks(range(len(top)))
	ax.set_yticklabels(top.index, fontsize=8)
	ax.invert_yaxis()
	ax.set_xlabel('Frequency')
	ax.set_title('Top 20 Skills in Job Listings (asaniczka, n={:,} entries)'.format(len(skills_df)))
	ax.spines['top'].set_visible(False)
	ax.spines['right'].set_visible(False)
	plt.tight_layout()
...
def fig6_job_level_distribution(asaniczka_df):
	...
	bars = ax.bar(range(len(levels)), levels.values,
		color=colors[:len(levels)], alpha=0.85, width=0.6, edgecolor='white', linewidth=1)
...
def main():
	...
	if sneha_df is not None and florex_df is not None:
		...
		ax.axvline(int(wc.median()), color=C_RED, linestyle='--',
			label='Median: {}'.format(int(wc.median())))
		ax.set_xlabel('Word Count')
		ax.set_ylabel('Frequency')
		ax.set_title('Resume Length Distribution (n={:,})'.format(len(wc)))
		ax.legend()
		ax.spines['top'].set_visible(False)
		ax.spines['right'].set_visible(False)
		plt.tight_layout()
	...
	print('\n' + '='*55)
	print('  Done! Generated {} figures in {}/'.format(len(generated), FIG_DIR))
	print('='*55)

	total_jobs    = arshkon_n + asaniczka_n
	total_resumes = sneha_n + florex_n
	print('\n  Total job listings:  {:,}'.format(total_jobs))
	print('  Total resumes:       {:,}'.format(total_resumes))
	print('  Combined corpus:     {:,} documents\n'.format(total_jobs + total_resumes))

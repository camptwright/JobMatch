...
def process_jobs(filepath):
	...
	print('  Raw records: {}'.format(len(df)))
	...
	print('  After dropping null descriptions: {}'.format(len(df)))
	...
	print('  After dropping short descriptions (<20 words): {}'.format(len(df)))
	...
	print('  After deduplicating identical descriptions: {} (removed {})'.format(
		len(df), before_dedup - len(df)))

	print('  Final job postings: {}'.format(len(df)))
	return df
...
def process_resumes(filepath):
	...
	print('  After dropping short resumes (<30 words): {}'.format(len(df)))

	df['Category'] = df['Category'].str.strip()

	print('  Final resumes: {}'.format(len(df)))
	print('  Categories: {}'.format(sorted(df['Category'].unique())))
	return df
...
def save_category_map():
	...
	print('\nCategory map saved to {}'.format(outpath))
...
def main():
	...
	for path, name in [(jobs_path, 'Job postings'), (resumes_path, 'Resumes')]:
		if not os.path.exists(path):
			print('ERROR: {} file not found at {}'.format(name, path))
			print('  Download from Kaggle and place in {}/'.format(RAW_DIR))
			return
	...
	print('  Jobs:    {:,} records -> {}'.format(len(jobs_df), jobs_out))
	print('  Resumes: {:,} records -> {}'.format(len(resumes_df), resumes_out))
	print('='*50)
